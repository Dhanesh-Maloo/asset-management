...
const Profile = () => {
  ...
  const statusColor = (s) => {
    const map = { available: 'bg-green-100 text-green-700', assigned: 'bg-blue-100 text-blue-700', in_use: 'bg-purple-100 text-purple-700', under_maintenance: 'bg-yellow-100 text-yellow-700', disposed: 'bg-red-100 text-red-700' };
    return map[s] || 'bg-slate-100 text-slate-700';
  };

  const priorityColor = (p) => {
    const map = { low: 'bg-slate-100 text-slate-600', medium: 'bg-blue-100 text-blue-700', high: 'bg-orange-100 text-orange-700', critical: 'bg-red-100 text-red-700' };
  ...
  return (
    <DashboardLayout>
      <div className="p-6 md:p-10 max-w-5xl mx-auto">
        <h1 className="text-3xl font-bold font-heading mb-6">My Profile</h1>
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          ...
          <Card className="text-center p-4">
            <div className="text-2xl font-bold text-green-600">{profile?.recent_orders?.length || 0}</div>
                  ...
                  {profile.open_tickets.map(ticket => (
                    <div key={ticket.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs px-2 py-0.5 rounded-full bg-orange-100 text-orange-700 font-medium">{ticket.status}</span>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="space-y-2">
                  {profile.recent_orders.map(order => (
                    <div key={order.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${order.status === 'approved' || order.status === 'fulfilled' ? 'bg-green-100 text-green-700' : order.status === 'rejected' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'}`}>
                        {order.status}
                      </span>
                    </div>
                  ))}
